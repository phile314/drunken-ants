...
class Compilable c => Jumpable c where
  ...
  -- | @'compileWithJump' xs j@ compiles the 'Compiable' elements contained in @xs@,
  -- so that on normal execution they are executed sequentially and after that
  -- the control flow jumpt to @j@.
