...
-- Upon a lookup the first level is used, if it fails the second outer level is used and so on, until either
-- a match is found or all the levels all searched.
...
-- | The lookup is performed first in the current level and in case of failures to the upper level.
-- If all the levels are consumed without any match Nothing is returned.
