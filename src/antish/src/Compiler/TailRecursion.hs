...
checkTailRecursive :: 
  ...
  -> Compile CState ()
checkTailRecursive iden b recs = tailRec >>= (`unless` raiseError)
  where tailRec = isTailRecursive b recs
        raiseError = throwError $ NotTailRecursive iden b
...
oneIsTailRecursive b1 b2 recs = liftM2 (||) t1 t2
  where t1 = isTailRecursive b1 recs
